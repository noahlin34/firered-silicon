...
1. **Downstairs Stair Warp (2F ↔ 1F): Complete**
   ...
   - **Object-event sprite sheets:** each `graphics/object_events/pics/**/<name>.png` must be converted with `-mwidth 2 -mheight 4` (canonical `spritesheet_rules.mk` layout). `mom.4bpp` was previously row-major (3 horizontal 16x32 frames), producing garbled frames; `Makefile.native` assets now regenerate it frame-major and `event_object_movement.o` depends on the generated `.4bpp`/`.gbapal` files directly. New NPC graphics need the same treatment plus a `.pal → .gbapal` rule.
...
2. **Pallet Town Exterior & Front-Door Warp (1F ↔ Outdoors): Complete**
   ...
   - **Still stubbed / deliberately disabled while walking outside:**
     - **Oak's interception trigger is INERT.** Pallet Town's coord events at `(12,1)`/`(13,1)` (`PalletTown_EventScript_OakTriggerLeft/Right`, `VAR_MAP_SCENE_PALLET_TOWN_OAK == 0`) are generated with `.script = NULL`, so the player can walk to Route 1 without a Pokémon. Reason: the real cutscene's dependency closure (15 scripts) uses script commands the native compiler does not implement yet — `famechecker`, `textcolor`, `delay`, `playbgm`, `playse`, `addobject`, `opendoor`/`waitdooranim`/`closedoor`, `warp`, `waitstate` — plus movement commands `walk_in_place_faster_up`, `set_invisible`, `step_end` and Oak's lab-escort movements. Coord events MUST stay NULL rather than `sDummyScript` (fix #27).
     ...
     - **Outdoor NPC dialogue is stubbed.** `PalletTown_EventScript_SignLady` / `_FatMan` and Pallet Town's coord/bg scripts are not in `NATIVE_SCRIPT_ROOTS`, so interacting does nothing; `PalletTown_EventScript_OaksLabSign`/`_TownSign`/`_TrainerTips` are dummy scripts. Add a scene to `NATIVE_SCRIPT_ROOTS` in `tools/gen_map_data.py` to compile it (delete any matching stub in `overworld_stubs.c`).
3. **Overworld Scripts & Interaction — next:**
   - **Oak's Pallet Town interception cutscene** (the scene that normally plays when you try to leave town without a Pokémon): compile `PalletTown_EventScript_OakTrigger{Left,Right}` + closure natively, which needs the new `tools/gen_map_data.py` command encodings listed in item 2, then flip those coord events from NULL to the real script.
