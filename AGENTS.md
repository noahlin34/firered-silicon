...
1. **Downstairs Stair Warp (2F ↔ 1F): Complete**
   ...
   - **How to extend to other scenes (TV, PC, signposts):** add the scene's script label(s) to `NATIVE_SCRIPT_ROOTS` in `tools/gen_map_data.py` and extend `ScriptRegistry.compile()` with the missing opcodes the closure needs (current subset: goto/call/branches, flags, vars, msgbox, lock/lockall/faceplayer/release, waitmessage/waitbuttonpress, applymovement/waitmovement, fanfare, fadescreen, special). The TV script (`PalletTown_PlayersHouse_1F_EventScript_TV`) uses `lockall`/`goto_if_eq VAR_FACING`/`checkplayergender` — all already supported; it fails only because it is not in `NATIVE_SCRIPT_ROOTS`. The 2F PC needs `EventScript_PC` wired via `gMapHeader`-style bg event — the generated `.bgUnion.script` currently emits `sDummyScript` for every non-root event.
...
3. **Overworld Scripts & Interaction:**
   - Mom dialogue and downstairs interaction are real; signpost/PC wiring is next (see dialogue notes above).
